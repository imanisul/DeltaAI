...
function SideBar() {
  ...
  if (collapsed) {
    return (
      ...
      >
        ...
        <div
          className="flex-1 overflow-y-auto px-2.5 pb-2 [scrollbar-width:none] [&
        ...
        >
          {conversations.map((conv, i) => {
            ...
            ${
              ...
              >
                <div
                  className={`flex items-center justify-center shrink-0 w-[20px] h-[20px] 
         ...
         <div className="relative shrink-0">
                {userData?.avatar && !imageError ? (
                  <img
                    className="w-9 h-9 rounded-[16px] object-cover border-2 border-indigo-500/25"
                    src={userData?.avatar}
                    alt={"image"}
  ...
  return (
    <div className="fixed lg:static inset-y-0 left-0 z-50 w-[270px] h-screen shrink-0 bg-[#0d014] border-r border-white/[0.06]">
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]">
          <div
            className="hidden lg:flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 
            hover:text-slate-200 hover:bg-white/[0.05] transition-colors duration-500 bg-transparent border-none cursor-pointer"
          ...
          <span
            ...
            border-indigo-500/20 px-2 y-0.5 rounded-full tracking-wide"
          >
            free
          </span>
    ...
    ::-webkit-scrollbar]:hidden"
        >
          {conversations.map((conv, i) => {
            ...
            ${
              ...
              >
                <div
                  className={`flex items-center justify-center shrink-0 w-[28px] h-[28px] 
                rounded-lg transition-colors duration-150
             ${isActive ? "bg-indigo-500/15 text-indigo-400" : "bg-white/[0.05] text-slate-500"}`}
                >
                  <MessageSquare size={13} />
        ...
        <div className="px-3.5 py-3.5">
          {userData ? (
            ...
            <button
              className="flex items-center justify-center w-7 text-sm font-medium
               text-slate-200 bg-white/[0.05] border border-white/[0.08] rounded-xl py-[11px] cursor-pointer 
               hover:bg-white/[0.08]  transition-color duration-150"
            >
              Login
            </button>
          )}
